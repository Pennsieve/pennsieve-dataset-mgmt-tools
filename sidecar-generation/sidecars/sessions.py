...
class SessionSidecar(TSVSidecar):
    ...
    - Defines required fields and defaults for each session row
    ...
    # Default values for session rows - user data overrides these
    ROW_DEFAULTS = {
        "acq_time": "n/a",
    ...
    def __init__(self, rows: List[Dict[str, Any]] = None, **kwargs):
        """
        Initialize SessionSidecar with optional row data.

        Args:
            rows: List of dicts, each representing a session row.
    ...
    def add_row(self, row: Dict[str, Any]) -> None:
        """Add a single session row, merging with defaults."""
        merged_row = {**self.ROW_DEFAULTS, **row}
        self.rows.append(merged_row)
